	...
	gamesDs.update(
		...
		},
		{
			$set: {
				...
				state:
					dirtyStats.venue &&
					((dirtyStats.venue.state && dirtyStats.venue.state.abbreviation) ||
						''),
				teams: [
					{
						score: dirtyStats.teams[0].score,
						name: dirtyStats.teams[0].title,
						location: dirtyStats.teams[0].location.type
					},
					{
						score: dirtyStats.teams[1].score,
						name: dirtyStats.teams[1].title,
						location: dirtyStats.teams[1].location.type
					}
				]
			}
		},
		...
		function(err, numReplaced) {
			...
			for (i = 0; i <= 1; i++) {
				...
				toInsert.push({
					...
					poss:
						gameData.field_goals.made +
						0.475 * gameData.field_goals.attempted -
						gameData.rebounds.offensive +
						gameData.turnovers.total
				});
			}
